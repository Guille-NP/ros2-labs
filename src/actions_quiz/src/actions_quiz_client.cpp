...
public:
  ...
  : Node("my_action_client", node_options), goal_done_(false)
  {
    ...
    this->timer_ = this->create_wall_timer(std::chrono::milliseconds(500), std::bind(&MyActionClient::send_goal, this));
  }
...
private:
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Feedback received: %f", feedback->current_dist);
  }
  ...
  void result_callback(const GoalHandleDistance::WrappedResult & result)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "Total distance: %f", result.result->total_dist);
